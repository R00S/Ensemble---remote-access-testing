...
import '../widgets/global_player_overlay.dart' show BottomSpacing, GlobalPlayerOverlay;
...
class _AudiobookSeriesScreenState extends State<AudiobookSeriesScreen> {
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            onPressed: _isLoading || _audiobooks.isEmpty ? null : _playFirstBook,
                      ...
                      SizedBox(
                        ...
                        child: FilledButton.tonal(
                          onPressed: _isLoading || _audiobooks.isEmpty ? null : () => _showPlayOnMenu(context),
                      ...
                      SizedBox(
                        ...
                        width: 50,
                        child: FilledButton.tonal(
                          onPressed: _isLoading || _audiobooks.isEmpty ? null : _addAllToQueue,
                          style: FilledButton.styleFrom(
                            padding: EdgeInsets.zero,
                            shape: RoundedRectangleBorder(
                              borderRadius: BorderRadius.circular(12),
  ...
  Future<void> _playFirstBook() async {
    if (_audiobooks.isEmpty) return;
    ...
    // Find book in progress, or default to first
    Audiobook bookToPlay = _audiobooks.first;
    ...
    try {
      maProvider.setCurrentAudiobook(bookToPlay);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Playing ${bookToPlay.name}'),
  ...
  Future<void> _addAllToQueue() async {
    ...
    try {
      // Add first book to play if nothing is playing
      final firstBook = _audiobooks.first;
      maProvider.setCurrentAudiobook(firstBook);
      await maProvider.api?.playAudiobook(
        selectedPlayer.playerId,
        firstBook,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Playing ${widget.series.name}'),
            duration: const Duration(seconds: 1),
          ),
        );
      }
